...
import { getDisplayNameForUid } from "@/lib/adminDisplayName";
import CreateLeagueClient from "./createLeagueClient";
import { createLeagueAction } from "./createLeagueAction";
...
export default async function SuperAdminLeaguesPage({
  searchParams,
  ...
  const allLeagues: LeagueRow[] = (await Promise.all(
    leagueIds.map(async (id) => {
      ...
      const adminName = await getDisplayNameForUid(adminUserId);
            ...
            <>
              ...
              >
                ...
                <div style={{ justifySelf: "end" }}>
                  {hasFilters && (
                    <a href="/superadmin/leagues" className="btn btn--light" style={{ marginRight: 8 }}>
                      Reset
                    </a>
                  )}
                ...
                <li key={l.leagueId}>
                  ...
                  >
                    ...
                    <div className="col-view" style={{ justifySelf: "end", marginRight: 0 }}>
                      <Link href={`/superadmin/leagues/${l.leagueId}`} className="card-cta">
                        VIEW LEAGUE →
                      </Link>
                    </div>
                  </div>
