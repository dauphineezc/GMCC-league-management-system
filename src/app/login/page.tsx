...
function EmbeddedSignInPrompt({ next }: { next: string | null }) {
    ...
    <main style={{ padding: 20, display: "grid", gap: 10 }}>
      ...
      >
        ...
        <p style={{ margin: "0 0 24px", lineHeight: 1.5, color: "var(--muted)" }}>
          Sign-in opens the full app in a new tab so you can use email or Google sign-in without
          browser restrictions.
        </p>
        <button
          type="button"
          className="brand-btn auth-primary"
          onClick={() => openAppInNewTab(loginPath)}
        >
          Open sign in
        </button>
      </section>
...
function LoginContent() {
  ...
  if (embedded) {
    return <EmbeddedSignInPrompt next={next} />;
  }
