...
function LoginContent() {
  ...
  async function signInEmbeddedWithGoogle() {
    const { idToken, isNewUser, email } = await waitForEmbeddedGoogleAuth();
    if (isNewUser) {
      const prefillEmail = email ? `?email=${encodeURIComponent(email)}` : "";
      router.push(`/create-account${prefillEmail}`);
      return;
    }
  ...
  async function signInExistingWithProvider(provider: any) {
    if (isEmbedded()) {
      await signInEmbeddedWithGoogle();
      return;
    }
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              ...
              >
                ...
                <span style={{ whiteSpace: "nowrap" }}>
                  {embedded ? "Sign in with Google (popup)" : "Sign in with Google"}
                </span>
              </button>
            </div>
          </>
